...
class RegTrainer(Trainer):
        ...
        for epoch in range(self.start_epoch, args.max_epoch):
            ...
            if epoch % args.val_epoch == 0 and epoch >= args.val_start:
                if has_val:
                    ...
                    # No validation set - use test set for both evaluation and model selection
                    logging.info('No validation data found; running test epoch for evaluation and model selection.')
                    self.test_epoch(save_best=True)
                    # If test-based early stopping flagged, break the main training loop
                    if getattr(self, 'should_stop', False):
    ...
    def train_eopch(self):
        """Training loop for one epoch - FIXED gradient handling"""
        # Initialize meters
        epoch_ot_loss = AverageMeter()
        epoch_ot_obj_value = AverageMeter()
        epoch_wd = AverageMeter()
        ...
        # Log epoch summary - FIXED formatting
        if self.rank == 0:
            log_msg = 'Epoch {} Train, Det Loss: {:.6f}, Total Loss: {:.6f}, GAME0: {:.2f} MSE: {:.2f}, Cost {:.1f} sec'.format(
                self.epoch, epoch_det_loss.get_avg(), epoch_total_loss.get_avg(),
                epoch_game.get_avg(), np.sqrt(epoch_mse.get_avg()), time.time() - epoch_start)
            logging.info(log_msg)
            
            # Additional diagnostics for detection
            if self.args.task == 'detection':
                logging.info('Epoch {} Detection: HM_pos {:.6f}, HM_neg {:.6f}, Size {:.6f}, Off {:.6f}'.format(
                    self.epoch, epoch_hm_pos.get_avg(), epoch_hm_neg.get_avg(), 
                    epoch_size_loss.get_avg(), epoch_off_loss.get_avg()))
                
                if self.use_bg_suppress:
                    logging.info('Epoch {} BG Suppress: {:.6f}'.format(self.epoch, epoch_bg_suppress.get_avg()))
            
            # Gradient norm diagnostic
            try:
                logging.info('Epoch {} Grad Norm: {:.6f}'.format(self.epoch, epoch_grad_norm.get_avg()))
            except Exception:
                pass
    ...
    def _evaluate_detection_sample(self, heat_pred, offset_pred, sample=None, min_score=0.1):
        """Extract detections from prediction heatmaps
        ...
        for idx in range(B):
            ...
            # NEW: Dynamic threshold based on image statistics
            ...
            # NEW: Limit detections based on expected count
