...
class Crowd(data.Dataset):
    def __init__(self, root_path, crop_size=256,
        ...
        # self.RGBT_transform = transforms.Compose([
        #     transforms.ToTensor(),
        #     transforms.Normalize(
        #         mean=[0.492, 0.168, 0.430],
        #         std=[0.317, 0.174, 0.191]),
        # ])
        self.RGBT_transform = transforms.Compose([transforms.ToTensor()])
    ...
    def __getitem__(self, item):
        ...
        rgbt_path = gt_path.replace('GT', 'RGBT').replace('npy', 'png')

        RGB = cv2.imread(rgb_path)[..., ::-1].copy()
        T = cv2.imread(t_path)[..., ::-1].copy()
        RGBT = cv2.imread(rgbt_path)[..., ::-1].copy()

        if self.method == 'train':
            keypoints = np.load(gt_path)
            return self.train_transform(RGB, T, RGBT, keypoints)
    ...
    def train_transform(self, RGB, T, RGBT, keypoints):
        ht, wd, _ = RGB.shape
