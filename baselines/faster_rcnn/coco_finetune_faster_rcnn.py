...
Fine-tunes a pre-trained Detectron2 model on DroneRGBT dataset (COCO format)
"""
...
def register_coco_datasets(coco_root_dir="./coco_droneRGBT", modality="rgb"):
    ...
    if train_json_path.exists():
        ...
        print(f"✓ Registered {train_dataset_name}: {len(train_dicts)} images")
    else:
    ...
    if test_json_path.exists():
        ...
        print(f"✓ Registered {test_dataset_name}: {len(test_dicts)} images")
...
def setup_cfg(output_dir, train_dataset, num_classes=1, 
    ...
    # Use Faster R-CNN with R50 backbone
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fine-tune Detectron2 on COCO format DroneRGBT dataset")
